...
const PLACEHOLDER_CUSTOMER_EMAIL_DOMAIN = 'placeholder.bhautoprotect';

const buildLeadDisplayName = (lead: Lead | null): string | null => {
  if (!lead) {
    return null;
  }
...
const ensureCustomerAccountForPolicy = async (
  policyId: string,
  lead: Lead | null,
  existingCustomers: CustomerAccount[] = [],
  ...
  const fallbackEmail = normalizedEmail
    ? normalizedEmail
    : `policy-${policyId.toLowerCase()}@${PLACEHOLDER_CUSTOMER_EMAIL_DOMAIN}`;

  let account: CustomerAccount | undefined;
  if (normalizedEmail) {
    account = await storage.getCustomerAccountByEmail(normalizedEmail);
  }

  if (!account) {
    account = await storage.getCustomerAccountByEmail(fallbackEmail);
  }

  if (!account) {
    const passwordSource =
      typeof lead.id === 'string' && lead.id.trim().length > 0 ? lead.id : policyId;
    account = await storage.createCustomerAccount({
      email: fallbackEmail,
      passwordHash: hashPassword(passwordSource),
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" width="640" style="width:640px;max-width:94%;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 22px 48px rgba(15,23,42,0.1);">
            ...
            <tr>
              <td style="padding:32px;">
                ...
                <p style="margin:0 0 18px;font-size:15px;line-height:1.7;">
                  Need adjustments? Reply to this email or call <a href="tel:+18339400234" style="color:#2563eb;text-decoration:none;font-weight:600;">(833) 940-0234</a>. Our concierge team can update payment schedules, deductibles, and start dates instantly.
                </p>
            ...
            <tr>
              <td style="padding:32px;text-align:center;">
                ...
                <p style="margin:0 0 16px;font-size:15px;line-height:1.7;text-align:center;">
                  Need help or have questions? Call <a href="tel:+18339400234" style="color:#2563eb;text-decoration:none;font-weight:600;">(833) 940-0234</a> or reply to this email. Our concierge team is ready to help with claims, maintenance advice, or billing adjustments.
                </p>
            ...
            <tr>
              <td style="padding:32px;">
                ...
                <p style="margin:24px 0 0;font-size:14px;line-height:1.7;color:#475569;">
                  Need a hand? Reply to this email or call <strong>(833) 940-0234</strong> and our team will help.
                </p>
              </td>
            </tr>
          </table>
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const adminPaymentProfileSchema = customerPaymentProfileSchema.extend({
    customerId: z.string().trim().min(1, 'Customer account is required').optional().nullable(),
  });
  ...
  app.post("/webhooks/leads", async (req, res) => {
    ...
    try {
      ...
      const leadInput: Partial<InsertLead> = {
        ...
        email,
        phone,
        affiliateId: normalizeInteger(payload.lsid),
        campaignId: normalizeInteger(payload.lcid),
        accessKey: normalizeString(payload.key),
        subId2: normalizeString(payload.subid2),
        subId3: normalizeString(payload.subid3),
        trustedFormCertUrl: normalizeString(payload.xxTrustedFormCertUrl),
        sourceUrl: normalizeString(payload.source_url),
        universalLeadId: normalizeString(payload.universal_leadid),
  ...
  app.post('/api/admin/leads/:id/convert', async (req, res) => {
    ...
    try {
      ...
      try {
        const account = await ensureCustomerAccountForPolicy(policy.id, lead, []);
        if (account) {
          ...
          const monthValue = monthRaw
            ...
            : Number.isFinite(lead.cardExpiryMonth)
              ? Number(lead.cardExpiryMonth)
              : null;
          const yearValue = yearRaw
            ? Number.parseInt(yearRaw, 10)
            : Number.isFinite(lead.cardExpiryYear)
              ? Number(lead.cardExpiryYear)
              : null;
  ...
  app.post('/api/admin/policies/:id/payment-profiles', async (req, res) => {
    ...
    try {
      ...
      const requestedCustomerId = trimmedCustomerId.length > 0 ? trimmedCustomerId : null;

      let account: CustomerAccount | null = null;

      if (requestedCustomerId) {
        const existing = await storage.getCustomerAccount(requestedCustomerId);
        if (!existing) {
        ...
        account = await ensureCustomerAccountForPolicy(
          policy.id,
          policy.lead ?? null,
          policy.customers ?? [],
        );
        if (!account) {
          res
            .status(400)
            .json({ message: 'Associate the policy with a lead before saving payment details' });
          return;
        }
      }
